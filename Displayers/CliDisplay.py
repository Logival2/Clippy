...
class CliDisplay(object):
    ...
    def draw_map(s, map_handler):
        ...
        to_add_top = needed_lines_top - map_handler.view_target.y
        ...
        squares_to_add_left = needed_squares_left - map_handler.view_target.x
        ...
        map_x_end = map_handler.view_target.x + avail_squares_right + 1


        # map_center = s.map_screen_size // 2
        final_player_pos = Pos(
                            term_y_idx + map_handler.player_pos.y - map_y_idx,
                            shift_x + (map_handler.player_pos.x - map_x_start) * 2
                        )

        delta = (final_player_pos - s.map_screen_size // 2)
        s.stdscr.addstr(s.screen_size.y, 1, f"{delta}      ")
        s.stdscr.addstr(s.screen_size.y, 50, f"{final_player_pos}       ")
        # s.stdscr.addstr(s.screen_size.y, 100, f"{delta}")
        # if delta > Pos(2, 4) or delta < Pos(-2, -4):
        map_handler.view_target.y += delta.y // 4
        map_handler.view_target.x += delta.x // 8
        ...
        while term_y_idx < s.map_screen_size.y:
            ...
            term_y_idx += 1
            map_y_idx += 1
        odpozjis = s.map_screen_size // 2
        s.map_scr.addstr(odpozjis.y, odpozjis.x, "X")
